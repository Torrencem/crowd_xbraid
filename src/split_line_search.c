...
int line_search_sync(braid_App app, braid_SyncStatus status) {
    ...
    if (iter != 0) {
        ...
        MPI_Recv(us_prev_message, bvector_size * my_num_values, MPI_BYTE, rank, TAG_US_PREV, MPI_COMM_WORLD, &mpi_status);

        // Unpack us_prev from us_prev_message
        braid_BaseVector *us_prev = malloc(sizeof(braid_BaseVector) * my_num_values);
        for (int i = 0; i < my_num_values; i++) {
        ...
        braid_BaseVector *us_fut = malloc(sizeof(braid_BaseVector) * my_num_values);
        for (int i = 0; i < my_num_values; i++) {
        ...
        while (fabs(c - d) > 0.00001) {
            ...
            double res_with_c;
            MPI_Allreduce(&my_res_with_c, &res_with_c, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            double res_with_d;
            MPI_Allreduce(&my_res_with_d, &res_with_d, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (int i = 0; i < my_num_values; i++) {
        _braid_BaseBufPack(core, app, us[i], us_prev_message + i * bvector_size, (braid_BufferStatus) core);
        free(us[i]);
    }
    MPI_Isend(us_prev_message, bvector_size * my_num_values, MPI_BYTE, rank, TAG_US_PREV, MPI_COMM_WORLD, &request);

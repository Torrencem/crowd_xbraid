...
void vec_axpy(int size, double alpha, double *x, double *y) {
    int i;
    for (i = 0; i < size; i++) {
        y[i] = y[i] + alpha * x[i];
    }
...
void apply_Phi(double dt, double *u) {
    u[0] = u[0] + dt * u[1];
    u[1] = u[1] - dt * u[1];
}
...
void apply_PhiAdjoint(double dt, double *w) {
    w[1] = w[1] - dt * w[1] + dt * w[0];
}

/*------------------------------------*/

void apply_Uinv(double dt, double *u) {
    u[0] /= 2 * dt;
    u[1] /= 2 * dt;
}
...
void apply_Vinv(double dt, double gamma, double *v) { v[0] /= 2 * gamma * dt; }

/*------------------------------------*/

/* Maps v to w (which may be the same pointer) */
void apply_D(double dt, double *v, double *w) {
    double vtmp = v[0];
    w[0] = 0.0;
    w[1] = -dt * vtmp;
}
...
void apply_DAdjoint(double dt, double *w, double *v) { v[0] = -dt * w[1]; }
...
int my_TriSolve(braid_App app, braid_Vector uleft, braid_Vector uright,
    ...
    if (uleft != NULL) {
        ...
        /* At the leftmost point, use a different center coefficient
         * approximation */
        rtmp[0] = -rtmp[0] * (2 * dt);
...
int my_Access(braid_App app, braid_Vector u, braid_AccessStatus astatus) {
    ...
    //
    //      braid_AccessStatusGetTIndex(astatus, &index);
    //      sprintf(filename, "%s.%02d.%04d.%03d", "trischur-ex-04.out", iter,
    //      index, app->myid); file = fopen(filename, "w"); fprintf(file,
    //      "%1.14e, %1.14e\n", (u->values)[0], (u->values)[1]); fflush(file);
    //      fclose(file);
    //   }
...
int main(int argc, char *argv[]) {
    ...
    while (arg_index < argc) {
        if (strcmp(argv[arg_index], "-help") == 0) {
            printf("\n");
            printf(" Solves a simple optimal control problem in time-serial on "
                   "[0, 1] \n\n");
            ...
            printf("  -gamma <gamma>          : Relaxation parameter in the "
                   "objective function \n");

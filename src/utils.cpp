...
Vector::Vector(const int len_) : len(len_) {
...
const Vector Vector::operator+(const Vector &m2) {
...
Tridiag_Matrix::Tridiag_Matrix(int n_) : n(n_) {
    this->al = new double[n - 1] { 0.0 };
    this->a = new double[n] { 0.0 };
    this->au = new double[n - 1] { 0.0 };
}

Tridiag_Matrix::Tridiag_Matrix(const Tridiag_Matrix &m2) {
    this->n = m2.n;
...
Vector Tridiag_Matrix::operator*(const Vector &x) {
    assert(x.len == n);
    Matrix as_mat(*this);
    return as_mat * x;
}
...
Matrix::Matrix(const int m_, const int n_) : m(m_), n(n_) {
...
Matrix::Matrix(const Tridiag_Matrix &m2) {
    this->m = m2.n;
    this->n = m2.n;
...
double &Matrix::element(const int row, const int col) {

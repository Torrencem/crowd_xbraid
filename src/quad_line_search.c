...
int line_search_sync(braid_App app, braid_SyncStatus status) {
    ...
    if (iter != 0) {
        ...
        braid_BaseVector *us_prev =
            (braid_BaseVector *) malloc(sizeof(braid_BaseVector) * my_num_values);
        for (int i = 0; i < my_num_values; i++) {
        ...
        for (int i = 0; i < my_num_values; i++) {
            ...
            _braid_BaseSum(core, app, 1.0, us_prev[i], 0.0, us[i]);
        }
        // Compute our part of the residual
        double my_res_with_0 = compute_total_residual(core, level);
        double r0;
        MPI_Allreduce(&my_res_with_0, &r0, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);

        // Update our us values to be with alpha = d
        for (int i = 0; i < my_num_values; i++) {
            _braid_BaseSum(core, app, 1.0, us_fut[i], 0.0, us[i]);
            _braid_BaseSum(core, app, 0.0, us_prev[i], 1.0, us[i]);
        }
        // Compute our part of the residual
        double my_res_with_1 = compute_total_residual(core, level);
        double r1;
        MPI_Allreduce(&my_res_with_1, &r1, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);

        for (int i = 0; i < my_num_values; i++) {
            _braid_BaseSum(core, app, 1.0, us_fut[i], 0.0, us[i]);
            _braid_BaseSum(core, app, 0.5, us_prev[i], 0.5, us[i]);
        }
        // Compute our part of the residual
        double my_res_with_1_2 = compute_total_residual(core, level);
        double r1_2;
        MPI_Allreduce(&my_res_with_1_2, &r1_2, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);

        double a = 2*r1*r1 + 2*r0*r0 - 4*r1_2*r1_2;
        double b = 4*r1_2*r1_2 - r1*r1 - 3*r0*r0;
        double alpha = -b/(2*a);
        if (rank == 0)
            printf("alpha = %f\n", alpha);

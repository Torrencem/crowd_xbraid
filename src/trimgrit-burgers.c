...
void apply_AInverse(int M, double *ac, double *al, double *au, double *u) {
    double *v = al;
    double li, lisave;
    int i;
...
void apply_A(int M, double *ac, double *al, double *au, double *u, double *v) {
    int i;
...
void compute_A(my_App *app, double dt, double *u, double *ac, double *al,
               double *au) {
...
void apply_Phi(my_App *app, double dt, double *g, double *u) {
    int M = (app->mspace);
    double *scr = (app->scr);
    double *ac = scr;
    double *al = scr + M;
...
void apply_DuPhi(my_App *app, double dt, double *u, double *w, int adjoint) {
    int M = (app->mspace);
    double *scr = (app->scr);
    double *ac = scr;
    double *al = scr + M;
...
void apply_DvPhi(my_App *app, double dt, double *u, double *w, int adjoint) {
    int M = (app->mspace);
    double *scr = (app->scr);
    double *ac = scr;
    double *al = scr + M;
...
void apply_GuObjective(my_App *app, double dt, double *u) {
    double dx = (app->dx);
    int M = (app->mspace);
    double *u0 = (app->u0);
...
void apply_GvObjective(my_App *app, double dt, double *v) {
    double dx = (app->dx);
...
int my_TriSolve(braid_App app, braid_Vector uleft, braid_Vector uright,
    ...
    for (iter = 0; iter < 1; iter++) {
        ...
        if (!xrelax) {
            /* Compute w_i using a Schur-complement approach on the 4x4 block
             * system. Assume the residual for the first equation is zero.  Use
             * diagonal of Schur-complement to scale residual.
             *
            ...
            apply_DvPhi(app, dt, (u->uvals), vectmp,
                        0); // note: u needed for implicit
            vec_axpy(mspace, -1.0, vectmp, 1.0, rtmp);
...
int main(int argc, char *argv[]) {
    ...
    while (arg_index < argc) {
        if (strcmp(argv[arg_index], "-help") == 0) {
            ...
            printf("        u(x,0) = u0(x) \n");
            printf("  -adv                    : Advection-diffusion problem "
                   "constraint\n");
            printf(
                "  -ode                    : Simple ODE problem constraint\n");
    ...
    /* Changed: Set to use line search after each iteration */
